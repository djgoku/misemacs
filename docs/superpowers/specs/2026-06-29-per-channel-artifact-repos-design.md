...
- **A channel that never built.** Until a channel's first successful publish its artifact repo has
  no releases, so `…-emacs-<channel>@latest` resolves to nothing. This is expected and bounded: the
