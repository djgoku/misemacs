#!/usr/bin/env bash
# test-self-heal-precheck.sh — manually edit pkgs/emacs/lockfile.toml's sha
...
LOCKFILE="pkgs/emacs/lockfile.toml"
BACKUP="$(mktemp)"
cp "$LOCKFILE" "$BACKUP"
...
cleanup() {
    ...
    bash scripts/hydrate.sh pkgs/emacs >/dev/null 2>&1 || true
}
trap cleanup EXIT

# Manual edit: rewrite sha WITHOUT bumping or hydrating. Mirrors what a
# user does with `vim pkgs/emacs/lockfile.toml`.
...
SRC_HEAD=$(git -C pkgs/emacs/src rev-parse HEAD 2>/dev/null || echo "?")
...
if ! mise run build emacs >/tmp/self-heal-build.out 2>&1; then
...
NEW_SRC_HEAD=$(git -C pkgs/emacs/src rev-parse HEAD)
