...
# builders. Forces pkgs-emacs-master stale via a content change to build.toml
...
# pkgs-emacs-master builder (auto = true), the verb would either time out
...
BUILD_TOML="pkgs/emacs-master/build.toml"
BACKUP="$(mktemp)"
...
# Force pkgs-emacs-master stale by appending a no-op trailing newline. blake3
# of build.toml changes; pkgs-emacs-master (which lists build.toml in sources)
...
# Confirm setup: --dry-run reports pkgs-emacs-master would install.
if ! mise deps install --dry-run 2>&1 | grep -q 'Would install: pkgs-emacs-master'; then
    echo "FAIL setup: pkgs-emacs-master not stale after build.toml mutation"
    exit 1
fi
...
for verb in hydrate status; do
    if ! timeout 60 mise run "$verb" >"/tmp/cheap-verb-$verb.out" 2>&1; then
        rc=$?
        echo "FAIL: mise run $verb exited $rc (likely auto-prelude fired pkgs-emacs-master builder)"
...
echo "PASS: test-cheap-verbs-fast — hydrate, status, doctor each completed without firing pkgs-emacs-master builder"
